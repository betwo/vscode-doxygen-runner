...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('extension.doxygen-runner.view_doxygen', (filepath) => {
      const config = vscode.workspace.getConfiguration('doxygen_runner');
      const use_live_preview = config['view_in_live_preview'] && vscode.extensions.getExtension("ms-vscode.live-server") !== undefined;
      try {
        let instance: Doxygen = instance_manager.getInstance(filepath);
        if (use_live_preview) {
          const index = instance.getIndexHtmlPath();
          vscode.commands.executeCommand('livePreview.start.preview.atFile', index);
        } else {
          instance.updateView();
        }
      } catch(err) {
